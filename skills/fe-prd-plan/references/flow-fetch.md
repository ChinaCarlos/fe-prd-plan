...
- **路径确认**：落到当前工作项目（不是 `fe-prd-plan` 自身）。Agent 必须询问用户归档位置。
    - 用户提供“需求简称”或“版本号”：默认落到 `docs/prd/<归档目录名>/`。
    - 用户提供“绝对/相对路径”：按用户指定的路径落盘（如 `specs/prd/`）。
- **文件组成**：正文存为 `.md`，图片存到同目录 `assets/`。
- **冲突处理**：归档已存在时提示复用，不覆盖，除非用户要求刷新。
