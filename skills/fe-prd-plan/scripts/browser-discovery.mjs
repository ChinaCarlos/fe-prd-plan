...
//   2. config.env 里 FE_PRD_PLAN_BROWSER 设了 → 严格模式，找不到则硬错
...
const CONFIG_PATH = path.join(SKILL_ROOT, 'scripts', 'config.env');
...
export async function selectBrowser(override = null) {
  ...
  const configured = readConfig().FE_PRD_PLAN_BROWSER || null;
