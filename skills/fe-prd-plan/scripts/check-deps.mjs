...
const CONFIG_PATH = path.join(SCRIPTS_DIR, 'config.env');
const CONFIG_TEMPLATE = path.join(SCRIPTS_DIR, 'templates', 'config.env.template');
...
function startProxyDetached(browserOverride) {
  const logFile = path.join(os.tmpdir(), 'fe-prd-plan-cdp-proxy.log');
...
async function ensureProxy(expectedBrowserId, browserOverride) {
  ...
  console.log(`  日志：${path.join(os.tmpdir(), 'fe-prd-plan-cdp-proxy.log')}`);
  return false;
}
...
async function resolveAndReport(override) {
  ...
  switch (result.kind) {
    ...
    case 'ambiguous': {
      ...
      console.log('  请询问用户：哪个浏览器作为 Agent 的默认？（写入 scripts/config.env 的 FE_PRD_PLAN_BROWSER）');
    ...
    case 'mismatch': {
      ...
      if (result.source === 'preference') {
        console.log(`  也可以编辑 scripts/config.env 改默认偏好，或本次临时换浏览器：node check-deps.mjs --browser <id>`);
      }
